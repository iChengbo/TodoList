...
export default {
  ...
  methods: {
    ...
    toChecked (todo, index) {
      ...
      axios.get('http://192.168.135.8:8082/api/update', {params: {id: todo._id, message: todo.message, isCompleted: !todo.isCompleted}}).then(function (res) {
    ...
    unEdit (todo) {
      todo.status = false
      axios.get('http://192.168.135.8:8082/api/update', {params: {id: todo._id, message: todo.message, isCompleted: false}}).then(function (res) {
    ...
    deleteTodo (item) {
      let _this = this
      axios.get('http://192.168.135.8:8082/api/delete', {params: {id: item._id}}).then(function (res) {
        if (res.status === 200) {
    ...
    deleteAllCompleted () {
      ...
      axios.get('http://192.168.135.8:8082/api/deleteAllCompleted').then(function (res) {
        if (res.status === 200) {
          for (let i = 0; i < len; i++) {
